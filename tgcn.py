...
class GCNBlock(nn.Module):
    def __init__(self, in_channels, spatial_channels, num_nodes, gcn_type, normalize, A):
        super(GCNBlock, self).__init__()
        GCNUnit = {'sage': SAGENet, 'gat': GATNet,
                   'gated': GatedGCNNet}.get(gcn_type)
        self.gcn = GCNUnit(in_channels=in_channels,
                           out_channels=spatial_channels, normalize=normalize, num_nodes=num_nodes, A=A)
...
class TGCN(nn.Module):
    def __init__(self, num_nodes, num_edges, num_features,
                 ...
                 A, gcn_type='sage', hidden_size=64, normalize='none',  **kwargs):
        """
